...
<div class="row">
		...
		<form class="form form-horizontal mar-top" action="{{route('products.update', $product->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
			...
			<div class="panel">
				...
				<div class="panel-body">
                    ...
                    <div class="form-group">
                        <label class="col-lg-2 control-label">{{translate('Product Slug')}}</label>
                        <div class="col-lg-7">
                            <input type="text" class="form-control"   value="{{$product->slug}}" readonly>
                        </div><span class="btn btn-primary" onclick="slugModalClick()"><i class="fa fa-edit"></i></span>
                    </div>
    ...
    {{--Video Modal show--}}
    <div class="modal fade" id="slug_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-zoom product-modal" id="modal-size" role="document">
            <div class="modal-content position-relative">
                <div class="modal-body">
                    ...
                    <div class="">
                        <form action="{{route('products.slug.update')}}"  method="post">
                            @csrf
                            <div class="form-group">
                                <input type="hidden" name="product_id" id="product_id" value="{{$product->id}}">
                                <label class="col-lg-2 control-label">{{translate('Product Slug')}}</label>
                                <div class="mb-4 text-info font-italic"><i class="fa fa-info-circle text-info"></i> eg: this-is-test-product-slug-do-like-this</div>
                                <div class="col-lg-7">
                                    <input type="text" class="form-control" name="product_slug" id="product_slug" onkeyup="slugData()"  value="{{$product->slug}}" >
                                </div>
                                <div class="text-center">
                                    <button class="btn btn-primary"><i class="fa fa-rocket"></i> Save</button>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    function slugModalClick(){
        $('#slug_modal').modal('show');
    ...
    function slugData()
    {
        ...
        $.ajax({
            type: "GET",
            url: "{{url('/admin/products/slug/check')}}/" +productSlug,
            dataType: "text",
            success: function(msg){
                if (msg == 1) {
                    showAlert('danger', '<h4> Slug match with other product. Plz try another slug! </h4>');
                }
                //showFrontendAlert('success', 'Copied');
                //toastr.warning('Slug match with other product. Plz try another slug')
